...
@Component({
  selector: 'app-highchart-app',
  templateUrl: './highchart-app.component.html',
  styleUrls: ['./highchart-app.component.css']
})
export class HighchartAppComponent implements OnInit {
  title = 'dynamicchart';
  public objArr=[];
   arrAvailability=[];
   arrPerformance=[];
...
for(var i=0;i<this.arrDate.length;i++){
  let date=moment(this.arrDate[i]).format('MMM-Do-YY');
  this.newDatesArray.push(date)
}
console.log(this.newDatesArray)
}
data=[{
  name: 'Availability',
  data: this.arrAvailability

},
{
name: 'performance',
data: this.arrPerformance
},
{
 name: 'quality',
 data: this.arrQuality
},
{
 name: 'oEE',
 data: this.arrOee
